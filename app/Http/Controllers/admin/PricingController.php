...
class PricingController extends Controller
{
    public function index()
    {
        ...
        if (request()->ajax()) {
            return datatables()->of(Pricing::latest()->get())
    ...
    {
        if (!in_array('createPricing',\Request::get('permission'))) {
            return redirect('admin');
        }
        if ($request->input()) {
            ...
            $record = ($form_choice == 'edit') ? Pricing::findOrFail($id) : new Pricing ;
            ...
            if($form_choice == 'edit' || $form_choice == 'duplicate'){
                $content['record'] = Pricing::findOrFail($id);
            }
    ...
    {
        if (!in_array('deletePricing',\Request::get('permission'))) {
            return redirect('admin');
        }
        if ($request->input('checkbox_value')) {
            $id = $request->input('checkbox_value');
            for ($i=0; $i < count($id); $i++) {
                $data = Pricing::findorFail($id[$i]);
                $data->delete();
            }
